...
export default class DB {
  ...
  constructor(dbPath: string) {
    this.docMaxByteLength = 100 * 1024 * 1024 // 100M
    this.docAttachmentMaxByteLength = 200 * 1024 * 1024 // 200M
    this.dbpath = dbPath
  ...
  private checkDocSize(doc: Doc<any>): DBError | false {
  ...
  async get(name: string, id: string): Promise<DocRes | null> {
    try {
      ...
      console.error('get doc error:', e)
      return null
    }
  }

  async remove(name: string, doc: Doc<any> | string): Promise<DBError | DocRes> {
    try {
      let target
  ...
  async bulkDocs(name: string, docs: Array<Doc<any>>): Promise<DBError | Array<DocRes>> {
    ...
    try {
      ...
      console.error('bulk docs error:', e)
      //
    }
    return result
  }
  ...
  async allDocs(name: string, key: string | Array<string>): Promise<DBError | Array<DocRes>> {
    ...
    try {
      ...
      console.error('all docs error:', e)
      //
    }
    return result
  }
  ...
  async getAttachment(name: string, docId: string, len = '0'): Promise<Buffer | null> {
    try {
